 ...
 *
 * @param <T>
 *            the type of the items emitted by the Observablesub
 */
public abstract class Observable<T> implements ObservableSource<T> {
    ...
    @SchedulerSupport(SchedulerSupport.NONE)
    public final Disposable subscribe(Consumer<? super T> onNext, Consumer<? super Throwable> onError,
                                      Action onComplete, Action onEnd) {
        return subscribe(onNext, onError, onComplete, Functions.emptyConsumer(), onEnd);
    }
    /**
     * Subscribes to an ObservableSource and provides callbacks to handle the items it emits and any error or
     * completion notification it issues.
...
//        subscribe(ls);
//        return ls;
        return subscribe(onNext, onError, onComplete, onSubscribe, Functions.EMPTY_ACTION);
    }

    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.NONE)
    public final Disposable subscribe(Consumer<? super T> onNext, Consumer<? super Throwable> onError,
                                      Action onComplete, Consumer<? super Disposable> onSubscribe, Action onEnd) {
//        ObjectHelper.requireNonNull(onNext, "onNext is null");
//        ObjectHelper.requireNonNull(onError, "onError is null");
//        ObjectHelper.requireNonNull(onComplete, "onComplete is null");
//        ObjectHelper.requireNonNull(onSubscribe, "onSubscribe is null");
//        ObjectHelper.requireNonNull(onEnd, "onEnd is null");
        if(null==onNext){
        ...
        if(null==onEnd){
            onEnd=Functions.EMPTY_ACTION;
        }

        LambdaObserver<T> ls = new LambdaObserver<T>(onNext, onError, onComplete, onSubscribe, onEnd);
        subscribe(ls);
